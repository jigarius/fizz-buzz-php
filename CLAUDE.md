...
- **Coding standard: Drupal** (enforced by `phpcs.xml.dist` → `<rule ref="Drupal"/>`). This means 2-space indentation, docblocks on every class/method, and `TRUE`/`FALSE`/`NULL` uppercase. Match the existing style in `lib/`.
